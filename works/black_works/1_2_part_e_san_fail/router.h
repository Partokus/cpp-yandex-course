...
namespace Graph {
  ...
  std::optional<typename Router<Weight>::RouteInfo> Router<Weight>::BuildRoute(VertexId from, VertexId to) const {
    ...
    while (!vertices_by_weight.empty()) {
      const auto node = vertices_by_weight.extract(vertices_by_weight.begin());
      done_vertices.insert(node.value().second);

      for (const EdgeId edge_id : graph_.GetIncidentEdges(node.value().second)) {
        const auto& edge = graph_.GetEdge(edge_id);
        if (done_vertices.count(edge.to)) {
          continue;
        }
        if (!routes_internal_data_[edge.to] || routes_internal_data_[edge.to]->weight > node.value().first + edge.weight) {
          if (routes_internal_data_[edge.to]) {
            vertices_by_weight.erase({routes_internal_data_[edge.to]->weight, edge.to});
          }
          routes_internal_data_[edge.to] = RouteInternalData{.weight = node.value().first + edge.weight,
                                                             .prev_edge = edge_id};
