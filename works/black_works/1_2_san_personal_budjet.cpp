...
class BulkLinearUpdater : public BaseBulkOperation<MoneyState, BulkLinearUpdater>
{
public:
...
private:
    ...
    public:
        ...
        {
            node->data = (node->left.get() ? node->left.get()->data : Data()) + (node->right.get() ? node->right.get()->data : Data());
        }
