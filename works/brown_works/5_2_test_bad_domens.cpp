...
void TestFilter()
{
...

    {
        ...
        ASSERT(IsSubdomain(domain2, domain1));
    }
...
{
    {
        string s = "kek mek mak sak";
        vector<string_view> parts = Split(s, " ");
        vector<string_view> expect{"kek", "mek", "mak", "sak"};
        ASSERT_EQUAL(parts, expect);
    }
    {
        string s = "kek.mek.mak.sak";
        vector<string_view> parts = Split(s, ".");
...
void TestIsSubOrSuperDomain()
{
    {
        Domain domen1{"mak.sam.mem"};
        Domain domen2{"kok.mak.sam.mem"};
        Domain domen3{"mak.sam.mem.kok"};
        ASSERT_EQUAL(domen1.GetPartCount(), 3U);
        ASSERT_EQUAL(domen2.GetPartCount(), 4U);
        ASSERT(IsSubOrSuperDomain(domen1, domen2));
        ASSERT(IsSubOrSuperDomain(domen2, domen1));
        ASSERT(!IsSubOrSuperDomain(domen1, domen3));
        ASSERT(!IsSubOrSuperDomain(domen3, domen1));
        ASSERT(!IsSubOrSuperDomain(domen3, domen2));
        ASSERT(!IsSubOrSuperDomain(domen2, domen3));
    }
}

void TestDomainCheckerAbsorbtion()
{
    {
        ...
        DomainChecker dc{bad_domains.begin(), bad_domains.end()};

        bad_domains[0] = Domain{"joka"};

        ASSERT(not dc.IsSubdomain(Domain{"m.ya.ru"}));
    }
}
...
void TestAll()
{
    ...
    RUN_TEST(tr, TestIsSubOrSuperDomain);
    RUN_TEST(tr, TestDomainCheckerAbsorbtion);
}
