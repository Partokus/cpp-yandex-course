...
StopPtr ParseAddStopQuery(istream &is)
{
...
void Parse(istream &is, ostream &os)
{
    ...
    for (size_t i = 0U; i < data_base_requests_count; ++i)
    {
        ...
        if (first_word == "Stop")
        {
            StopPtr stop = ParseAddStopQuery(is);
...
Bus 258
        ...
        istringstream iss_expect(R"(Bus Malina Kalina: 6 stops on route, 5 unique stops, 4371.02 route length
Bus 750: 5 stops on route, 3 unique stops, 20939.5 route length
Bus 751: not found
Bus 257: 7 stops on route, 4 unique stops, 4446.15 route length
Bus 258: 7 stops on route, 4 unique stops, 4446.15 route length
)");
...
Bus 758
        ...
        istringstream iss_expect(R"(Bus 256: 3 stops on route, 2 unique stops, 1.642e+06 route length
Bus 750: 3 stops on route, 2 unique stops, 1.642e+06 route length
Bus 256: 3 stops on route, 2 unique stops, 1.642e+06 route length
Bus 751: not found
Bus 755: 3 stops on route, 2 unique stops, 1.642e+06 route length
Bus 756: 3 stops on route, 2 unique stops, 1.642e+06 route length
Bus 757: 7 stops on route, 4 unique stops, 4.92599e+06 route length
Bus 758: 7 stops on route, 4 unique stops, 3.28399e+06 route length
)");
...
Bus X
        ...
        istringstream iss_expect(R"(Bus X: 19 stops on route, 10 unique stops, 11017.9 route length
Bus X: 19 stops on route, 10 unique stops, 11017.9 route length
)");
